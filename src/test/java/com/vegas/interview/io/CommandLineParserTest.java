...
class CommandLineParserTest {
    ...
    void validArguments() {
        ...
        assertEquals(new BigDecimal(50), parser.getMinPrice());
        assertEquals(new BigDecimal("200.50"), parser.getMaxPrice());
    ...
    void invalidMinPrice() {
        ...
        assertFalse(parser.isValid());
        assertTrue(
                parser.getValidationErrors().contains("Invalid minimum price: -1. It must be a non-negative number."));
    }
    ...
    void invalidMaxPriceNegative() {
        ...
        CommandLineParser parser = new CommandLineParser(args);
        assertFalse(parser.isValid());
        assertTrue(parser.getValidationErrors().contains(
                "Invalid maximum price: -1. It must be a non-negative number, and not less than minimum price."));
    }
    ...
    void invalidMaxPriceLessThanMin() {
        ...
        assertFalse(parser.isValid());
        assertTrue(parser.getValidationErrors().contains(
                "Invalid maximum price: 20. It must be a non-negative number, and not less than minimum price."));
    }
